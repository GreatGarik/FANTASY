...
async def process_championship_full():
    ...
    await add_places_after_gp(points_list, await get_actual_gp_async())
...
async def statistic_team_excel():
    ...
    places_list: dict = await get_team_places_by_name(datetime.now().year)
    teams_fonts: dict = await get_teams_fonts_colors()
    ...
    for entry in points_list:
        ...
        # вычисляем Average по all_res — сумма / количество элементов (если all_res пуст, 0)
        all_res = entry.get('all_res') or []
        count = len(all_res)
        entry['Average'] = sum(all_res) / count if count else 0
    ...
    # --- лист Average per race (с колонкой Position справа) ---
    ws2 = wb.create_sheet("Average per race")
    header2 = ['Team', '', 'Average per race', 'POS APR']
    ws2.append(header2)
    ...
    for idx, entry in enumerate(avg_sorted, start=1):
        avg_value = entry.get('Average', 0)
        row = [entry.get('Team', ''), '', round(avg_value, 2), idx]
        ws2.append(row)
        ws2.row_dimensions[ws2.max_row].height = 17
        row_idx = ws2.max_row
    ...
    # колонки и выравнивание для второго листа (включая новую колонку Position)
    ...
    for cell in ws2['A'] + ws2['B'] + ws2['C'] + ws2['D']:
        cell.alignment = center_alignment
...
async def export_places_summary_by_year():
    rows = await get_user_places_by_year(datetime.now().year)
...
async def export_counts_to_excel():
    counts = await counts_selects(datetime.now().year)  # {"total_predicts": int, "drivers": [...], "teams": [...], "engines": [...]}
...
async def statistic_user_position_excel():
    ...
    def _make_image(path, percent):
        im = Image(path)
        im.width = int(im.width * (percent / 100))
    ...
    if places_list2:
        ...
        ws2.row_dimensions[3].height = 22.5
        ...
        ws2.cell(row=8, column=1).fill = PatternFill(start_color='FFC50D', end_color='FFC50D', fill_type='solid')
        ws2.cell(row=9, column=1).fill = PatternFill(start_color='A3A3A3', end_color='A3A3A3', fill_type='solid')
        ws2.cell(row=10, column=1).fill = PatternFill(start_color='BC5610', end_color='BC5610', fill_type='solid')
